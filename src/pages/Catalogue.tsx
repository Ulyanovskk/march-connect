...
import { Filter, Grid3X3, List, ChevronDown, X, Loader2, Plus } from 'lucide-react';
...
const Catalogue = () => {
    ...
    if (priceRange) {
      result = result.filter(product => {
        ...
        switch (priceRange) {
          ...
          case '300000-500000': return price >= 300000 && price <= 500000;
  ...
  useEffect(() => {
    setVisibleCount(12);
  }, [selectedCategory, selectedCities, searchQuery, priceRange, sortBy]);
      ...
      <main className="flex-1 bg-muted/20">
        <div className="container mx-auto px-4 py-8">
          <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black tracking-tight text-foreground">Notre Catalogue</h1>
              <p className="text-muted-foreground mt-1 font-medium">Découvrez {filteredAndSortedProducts.length} pépites sélectionnées pour vous</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="hidden md:flex items-center bg-white p-1 rounded-xl shadow-sm">
                <Button variant={viewMode === 'grid' ? 'secondary' : 'ghost'} size="icon" className="rounded-lg h-9 w-9" onClick={() => setViewMode('grid')}><Grid3X3 className="h-4 w-4" /></Button>
                <Button variant={viewMode === 'list' ? 'secondary' : 'ghost'} size="icon" className="rounded-lg h-9 w-9" onClick={() => setViewMode('list')}><List className="h-4 w-4" /></Button>
              </div>
            </div>
          </div>
                ...
                <Sheet>
                  <SheetTrigger asChild>
                    <Button variant="outline" className="flex-1 h-12 rounded-xl bg-white border-none shadow-sm gap-2 font-bold">
                      ...
                      {activeFiltersCount > 0 && <Badge className="ml-1 bg-primary text-primary-foreground rounded-full h-5 w-5 p-0 flex items-center justify-center">{activeFiltersCount}</Badge>}
                    </Button>
                  </SheetTrigger>
                  <SheetContent side="left" className="w-full sm:w-[400px]">
                    <SheetHeader className="mb-6"><SheetTitle className="text-2xl font-black">Filtres</SheetTitle></SheetHeader>
                    <FilterContent />
                  </SheetContent>
                </Sheet>
              </div>
            ...
            <div className="flex-1">
              {isLoading ? (
                ...
                <div className="space-y-10">
                  ...
                  {visibleCount < filteredAndSortedProducts.length && (
                    <div className="flex justify-center pb-8">
                      <Button variant="outline" size="lg" className="rounded-2xl px-12 font-bold bg-white shadow-soft transition-all hover:scale-105 active:scale-95 group" onClick={handleShowMore}>
                        <Plus className="w-5 h-5 mr-2 text-primary group-hover:rotate-90 transition-transform duration-300" />
                ...
                <div className="bg-white rounded-3xl py-20 px-6 text-center shadow-soft border border-white">
                  <div className="w-24 h-24 bg-muted/30 mx-auto mb-6 rounded-full flex items-center justify-center"><Filter className="h-10 w-10 text-muted-foreground" /></div>
                  <h3 className="font-bold text-2xl mb-2">Aucun résultat</h3>
                  <p className="text-muted-foreground mb-8 max-w-sm mx-auto">{searchQuery ? `Nous n'avons trouvé aucun produit correspondant à "${searchQuery}".` : "Nous n'avons trouvé aucun produit correspondant à vos filtres actuels."} Essayez d'élargir votre recherche.</p>
                  <Button size="lg" className="rounded-xl px-10 font-bold" onClick={clearFilters}>Réinitialiser tout</Button>
                </div>
              )}
            </div>
          </div>
